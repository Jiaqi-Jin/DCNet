...
def get_boundary_point_circle(y, x, angle):
    '''
    Given point y,x with angle, return a two point in image boundary with shape [H, W]
    return point:[x, y]
    '''
    ...
    if angle == -np.pi / 2:
        y1 = 200 - int(np.sqrt(80000 - (x - 200) * (x - 200)))
        y2 = 200 + int(np.sqrt(80000 - (x - 200) * (x - 200)))
        point1 = (x, y1)
        point2 = (x, y2)
    elif angle == 0.0:
        x1 = 200 - int(np.sqrt(80000 - (y - 200) * (y - 200)))
        x2 = 200 + int(np.sqrt(80000 - (y - 200) * (y - 200)))
        point1 = (x1, y)
        point2 = (x2, y)
    else:
        k = np.tan(angle)
        b = y - k * x
        t = (2*k*(b - 200) - 400)*(2*k*(b - 200) - 400)-4*(k*k + 1)*(b*b-400*b)
        if t > 0 or t == 0:
            x1 = int((-(2 * k * (b - 200) - 400) - np.sqrt(t)) / (2 * (k * k + 1)))
            x2 = int((-(2 * k * (b - 200) - 400) + np.sqrt(t)) / (2 * (k * k + 1)))
            y1 = int(k * x1 + b)
            y2 = int(k * x2 + b)
            point1 = (x1, y1)
            point2 = (x2, y2)
...
def point_circle(b_point):

    points = []
    point1 = None
    point2 = None

    if b_point[1] == b_point[3]:
        y1 = 200 - int(np.sqrt(80000 - (b_point[1] - 200) * (b_point[1] - 200)))
        y2 = 200 + int(np.sqrt(80000 - (b_point[1] - 200) * (b_point[1] - 200)))
        point1 = (b_point[0], y1)
        point2 = (b_point[0], y2)
    elif b_point[0] == b_point[2]:
        x1 = 200 - int(np.sqrt(80000 - (b_point[0] - 200) * (b_point[0] - 200)))
        x2 = 200 + int(np.sqrt(80000 - (b_point[0] - 200) * (b_point[0] - 200)))
        point1 = (x1, b_point[1])
        point2 = (x2, b_point[1])
    else:
        k = (b_point[2] - b_point[0])/(b_point[3] - b_point[1])
        b = b_point[0] - k * b_point[1]
        t = (2*k*(b - 200) - 400)*(2*k*(b - 200) - 400)-4*(k*k + 1)*(b*b-400*b)
        if t > 0 or t == 0:
            x1 = int((-(2 * k * (b - 200) - 400) - np.sqrt(t)) / (2 * (k * k + 1)))
            x2 = int((-(2 * k * (b - 200) - 400) + np.sqrt(t)) / (2 * (k * k + 1)))
    ...
    if point1 is not None and point2 is not None:
        points.append(point1[0])
        points.append(point1[1])
        points.append(point2[0])
        points.append(point2[1])
    return points
