...
class FeaturePyramidNetwork(nn.Module):
    ...
    def __init__(self, in_channels_list, out_channels, extra_blocks=None, layer_id=1, Multi_scale_merge=None):
        ...
        self.Multi_scale_merge = Multi_scale_merge
    ...
    def forward(self, x):
        ...
        # 多尺度特征图简单累加
        if self.Multi_scale_merge == "sum":
            out = results[self.layer_id]
            for i, r in enumerate(results[:-1]):
                if i == self.layer_id:
                    continue
                out += F.upsample(r, size=(out.size()[-2:]), mode="nearest")
        elif self.Multi_scale_merge == 'concat':
            out_ = results[self.layer_id]
            for i, r in enumerate(results[:-1]):
                results[i] = F.upsample(r, size=(out_.size()[-2:]), mode="nearest")
            out = torch.cat(results[:-1], dim=1)
        elif self.Multi_scale_merge == 'weighting_sum':
            # 待扩充
            print(self.Multi_scale_merge)
        else:  # “None” 多尺度特征图
            # make it back an OrderedDict
            # out.size:（2，256，200，200）（2，256，100，100）（2，256，50，50）（2，256，25，25）（2，256，13，13）
...
class BackboneWithFPN(nn.Module):
    ...
    def __init__(self, backbone, return_layers, in_channels_list, out_channels, layer_id, scale_merge):
...
def resnet_fpn_backbone(model_name, layer_id=1, scale_merge='sum'):
    ...
    out_channels = 256
    return BackboneWithFPN(resnet_backbone, return_layers, in_channels_list, out_channels, layer_id, scale_merge)

if __name__ == "__main__":
    model = resnet_fpn_backbone(model_name='resnet50', )
    print(model)