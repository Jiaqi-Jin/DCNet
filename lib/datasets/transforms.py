    ...
    def normalize_annotations(anns):
        ...
        for ann in anns:
            ...
            if "keypoints" not in ann.keys():
                print("image_id:" + str(ann["image_id"]))
                print("id:" + str(ann["id"]))
                anns.remove(ann)
            ann['keypoints'] = np.asarray(ann['keypoints'], dtype=np.float32).reshape(-1, 3)
            # ann['keypoints'] = np.delete(ann['keypoints'], [1, 2, 3], axis=0)
...
class Crop_aug(Preprocess):
    ...
    def crop(self, image, anns):
        w, h = image.size
        w_padding = int(self.radio * w / 2.0)  # 1350
        h_padding = int(self.radio * h / 2.0)  # 900
        x_offset, y_offset = 0, 0
        if w > self.radio * w:
            x_offset = torch.randint(-w_padding, w - w_padding, (1,))
            x_offset = torch.clamp(x_offset, min=0, max=w - self.radio * w).item()
        if h > self.radio * h:
            y_offset = torch.randint(-h_padding, h - h_padding, (1,))
            y_offset = torch.clamp(y_offset, min=0, max=h - self.radio * h).item()
        self.log.debug('crop offsets (%d, %d)', x_offset, y_offset)

        # crop image
        new_w = min(self.radio * w, w - x_offset)
        new_h = min(self.radio * h, h - y_offset)
        ltrb = (x_offset, y_offset, x_offset + new_w, y_offset + new_h)
        image = image.crop(ltrb)
...
class RandomRotate(Preprocess):
    ...
    def __call__(self, image, anns, meta):
        ...
        img_rot, R = self.rotate_bound(np.asarray(image), np.copy(degree), (124, 116, 104))
